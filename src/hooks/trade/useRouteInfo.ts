 ...
 */
export function useRouteInfoReverse(denomIn: string, denomOut: string, amountOut: BigNumber) {
  const chainConfig = useChainConfig()
  const isOsmosis = chainConfig.isOsmosis
  const { data: assets } = useAssets()
  ...
  const neutronReverseRoute = useSWR<SwapRouteInfo | null>(
    !isOsmosis && debouncedAmount !== '0' && assets && amountOut.gt(0)
      ...
      : null,
    async () => getNeutronRouteInfoReverse(denomIn, denomOut, amountOut, assets, chainConfig),
  )
